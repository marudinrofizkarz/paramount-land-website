...
export function AgentContactComponent({
  ...
  // Normalize single agent config
  const normalizeAgent = (agent: SingleAgent): SingleAgent => ({
    name: agent.name || "",
    title: agent.title || "",
    phone: agent.phone || "",
    photo: agent.photo || "",
  ...
  const normalizedConfig = {
    ...config,
    title: config.title || "Hubungi Sales Terpercaya Kami",
    subtitle:
      config.subtitle ||
      "Dapatkan konsultasi expert dari sales in-house terbaik",
    ctaText: config.ctaText || "Hubungi Sekarang",
    ctaWhatsappText: config.ctaWhatsappText || "WhatsApp",
    ctaEmailText: config.ctaEmailText || "Email",
