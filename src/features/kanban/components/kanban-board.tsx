...
export function KanbanBoard() {
  ...
  useEffect(() => {
    if (currentBoard && isMounted) {
      loadBoardData(currentBoard.id);
    }
  }, [currentBoard, isMounted]);
  ...
  const announcements: Announcements = {
    ...
    onDragOver({ active, over }) {
      ...
      if (
        ...
        if (active.data.current.task.columnId !== over.data.current.task.columnId) {
  ...
  async function onDragEnd(event: DragEndEvent) {
    ...
    if (isActiveATask && isOverATask) {
      ...
      if (activeTask.columnId !== overTask.columnId) {
        ...
        const overTasks = tasks.filter(task => task.columnId === overTask.columnId);
    ...
    if (isActiveATask && isOverAColumn) {
      ...
      if (activeTask.columnId !== overColumn.id) {
        const overTasks = tasks.filter(task => task.columnId === overColumn.id);
  ...
  function onDragOver(event: DragOverEvent) {
    ...
    if (isActiveATask && isOverATask) {
      ...
      if (activeTask && overTask && activeTask.columnId !== overTask.columnId) {
        ...
        useTaskStore.getState().setTasks(arrayMove(newTasks, activeIndex, overIndex));
      }
    }
    ...
    if (isActiveATask && isOverAColumn) {
      ...
      if (activeTask && activeTask.columnId !== overColumn.id) {
        const newTasks = [...tasks];
        newTasks[activeIndex] = { ...activeTask, columnId: overColumn.id };
